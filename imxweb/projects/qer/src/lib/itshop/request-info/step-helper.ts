...
import { EntityCollectionData, EntityData } from '@imx-modules/imx-qbm-dbts';
import { OrderedWorkingStep } from './ordered-working-step.interface';

export function buildWorkingStepsOrdered(
  ...
  const stepsForWorkingMethod =
    workflowSteps?.Entities?.filter((elem) => elem?.Columns?.UID_QERWorkingMethod.Value === workingMethod) ?? [];

  const startSteps =
    stepsForWorkingMethod.filter(
